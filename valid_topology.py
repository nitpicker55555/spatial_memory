...
for folder_name in os.listdir(base_path):
    ...
    if os.path.isdir(full_path):
        path_name = full_path
        print("Current path_name:", path_name)
        data_dir = Path(path_name)

        folder_key = folder_name
        edges_path = data_dir / f"{folder_key}.edges.json"
        locations_path = data_dir / f"{folder_key}.locations.json"

        if not edges_path.exists() or not locations_path.exists():
            print(f"Missing files in {folder_key}, skipping.")
            continue
